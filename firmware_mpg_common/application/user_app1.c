...
void UserApp1Initialize(void)
{
  u8 au8WelcomeMessage[] = "TOTAL         FAILED";
...
static void UserApp1SM_Idle(void)
{
	...
	u8 au8DataContent[] = "xxxxxxxxxxxxxxxxxxxx";
	u8 au8LCDTotalCount[]="xxxxx          xxxxx";
	...
	if( AntReadAppMessageBuffer() )
	{
		...
		for(u8 i = 0; i < 3; i++)
		{
		au8LCDTotalCount[2 * i]       = HexToASCIICharUpper(au8TestMessage[i+5] / 16);
		au8LCDTotalCount[2 * i + 1]   = HexToASCIICharUpper(au8TestMessage[i+5] % 16);
		au8LCDTotalCount[2 * (7+i)]   = HexToASCIICharUpper(au8TestMessage[i+1] / 16);
		au8LCDTotalCount[2 * (7+i)+1] = HexToASCIICharUpper(au8TestMessage[i+1] % 16);      
		}

		LCDMessage(LINE2_START_ADDR,au8LCDTotalCount);
	} /* end AntReadData() */
